...
/**
 * @name keythrough
 * @description A svelte action that allows you to navigate through a list of items using the arrow
 *              keys, built with menubars in mind.
 ...
 * @see
 * {@link https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Roles/menubar_role Menubar keyboard interactions}
 * @example <div use:keythrough={{query: '.item'}}>...</div>
 */
export default (node: HTMLElement, { targets = node.children }: { targets: Targets }) => {
	// collect nodes
...
const getNodes = (node: HTMLElement, targets: Targets) => {
	...
	if (Array.isArray(targets))
		nodes = targets; // array of nodes, as-is
